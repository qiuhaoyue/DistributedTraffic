 ...
 */
public class PostGISReader extends PostgresSource implements RoadReader,Serializable{
        ...
        if (result_set == null) {
            ...
            String query =
                    "SELECT gid,osm_id,class_id,source,target,"
                            + "length,reverse_cost,maxspeed_forward,maxspeed_backward,"
                            + "priority, ST_AsBinary(the_geom) as geom FROM " + table + where + ";";
        ...
        try {
            ...
            do {
                ...
                if (!config.containsKey(classId)) {
                	Common.logger.debug("class id not contained");
                    continue;
                }
                long source = Long.parseLong(result_set.getString(4));
                long target = Long.parseLong(result_set.getString(5));
                //judge closed ways, real length is referred to the_geom
                double temp_length = Double.parseDouble(result_set.getString(6));
                /*if(temp_length == 0){
                	Common.logger.debug("closed way,length 0");
                	continue;
                }*/
                // 1000;
                ...
                //int maxspeedForward =
                //        result_set.getObject(8) == null ? 50 : Integer
                //                .parseInt(result_set.getString(8));
                //int maxspeedBackward =
                //        result_set.getObject(9) == null ? 50 : Integer
                //                .parseInt(result_set.getString(9));
                float priority = (float) config.get(classId).one().doubleValue();
                //float priority = result_set.getFloat(10);
        	...
        	Common.logger.debug("read roadmap exception");

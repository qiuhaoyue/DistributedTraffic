...
public class RoadMap extends Graph<Road> implements Serializable {
    ...
    private class Index implements SpatialIndex<RoadPoint>, Serializable {
        ...
        private Set<RoadPoint> split(Set<Tuple<Integer, Double>> points) {
            ...
            for (Tuple<Integer, Double> point : points) {
            	//System.out.println("successor "+edges.get((long) point.one() * 2).successor);
    ...
    public static RoadMap Load(RoadReader reader) throws SourceException {
        ...
        Common.logger.debug("osmcounter: " + osmcounter + " ;counter: " + counter);
    ...
    public SpatialIndex<RoadPoint> spatial(HashMap<Long, Tuple2<Road,Road>> successorAndNeighbor) {
        if (index == null)
        {
        	...
        	index = new Index();
            for (Road road : edges.values()) {
            	road.successor = successorAndNeighbor.get(road.id())._1;
            	road.neighbor = successorAndNeighbor.get(road.id())._2;
            	index.put(road);
            }
            long collectEnd = System.currentTimeMillis();
            System.out.println("indexTime "+(collectEnd-collectStart));
            return index;
        	//throw new RuntimeException("index not constructed");
        }
        else

...
class Worker extends Thread{
	...
	void enqueue(Task task) {
		...
		if (logger.isTraceEnabled()) {
			logger.trace("enqueue task {} to worker {}", task.toString(),
					workerid);
		}
	...
	void execute() {
		...
		if (current != null && logger.isTraceEnabled()) {
			logger.trace("worker {} fetched task {} from local queue",
					workerid, current.toString());
		}
		...
		if (current == null) {
			...
			if (current != null && logger.isTraceEnabled()) {
				logger.trace("worker {} fetched task {} from global queue",
						workerid, current.toString());
			}
		}
		...
		if (current == null) {
			...
			while (!workers.isEmpty()) {
				...
				if (current != null) {
					if (logger.isTraceEnabled()) {
						logger.trace(
								"worker {} fetched task {} from remote queue {}",
								workerid, current.toString(), remoteid);
					}
					break;
				}

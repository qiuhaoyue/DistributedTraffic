...
public class KState<C extends StateCandidate<C, T, S>, T extends StateTransition, S extends Sample>
        extends StateMemory<C, T, S> implements Serializable{
    /**
	 * 
	 */
    ...
    public final LinkedList<Tuple<Set<C>, S>> sequence;
    public final Map<C, Integer> counters;
        ...
        for (int i = sequence.size() - 1; i >= 0; --i) {
            if (kestimate != null) {
            	...
            	try{
                	...
                	e.printStackTrace();
                	System.out.println("sequence size" + sequence.size() + " Set size: "+ sequence.get(i).one().size()
                			+ " Set" + sequence.get(i).one().toString());	
                }
    ...
    public C update_converge(Set<C> vector, S sample){
    	C converge = null;
    	update(vector, sample);
    	//get converge point if exists
    	if(sequence.isEmpty()){
    		//System.out.println("sequence empty");
    		return null;
    	}
    	if(sequence.peekFirst().one().size() == 1){
    		converge = sequence.pollFirst().one().iterator().next();
            counters.remove(converge);
    	...
    	while ((k >= 0 && sequence.size() > k + 1)) {
            ...
            //converge = deletes.iterator().next();
            for (C candidate : deletes) {
                if (converge == null || candidate.seqprob() > converge.seqprob()) {
                	converge = candidate;
                }
            }
        }

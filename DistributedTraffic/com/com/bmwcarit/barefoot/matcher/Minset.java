 ...
 */
public abstract class Minset implements Serializable{
     ...
     */
    public static Set<RoadPoint> minimize(HashMap<Long, Tuple2<Road,Road>> successorAndNeighbor,Set<RoadPoint> candidates) {
        ...
        for (RoadPoint candidate : candidates) {
        	//修改后
            Iterator<Road> successors = candidate.edge().successors(successorAndNeighbor);
        	//System.out.println("successor "+candidate.edge().successor);
            ...
            while (successors.hasNext()) {
                ...
                if (!map.containsKey(successor.id())) {
                	//System.out.println("misses "+id+" "+ (misses.get(id) + 1));
                    misses.put(id, misses.get(id) + 1);
                }

                if (map.containsKey(successor.id()) && map.get(successor.id()).fraction() == 0) {
                	//System.out.println("removes "+successor.id()+" id "+id+" "+ (misses.get(id) + 1));

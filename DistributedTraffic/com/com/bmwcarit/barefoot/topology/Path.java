 ...
 */
public class Path<E extends AbstractEdge<E>> implements Serializable{
    /**
	 * 
	 */
    ...
    //add 0423
    public Path(HashMap<Long, Tuple2<Road,Road>> successorAndNeighbor,Point<E> source, Point<E> target, List<E> edges) {
        this.source = source;
        this.target = target;
        this.edges = new LinkedList<E>(edges);
        //this.mapEdges = mapEdges;
        if (!valid(successorAndNeighbor)) {
            throw new RuntimeException("unvalid path");
        }
    }
     ...
     */
    public boolean valid(HashMap<Long, Tuple2<Road,Road>> successorAndNeighbor) {
        ...
        for (int i = 0; i < edges.size() - 1; i++) {
        	Iterator<E> successors = edges.get(i).successors(successorAndNeighbor);
    ...
    boolean valid() {
        ...
        for (int i = 0; i < edges.size() - 1; i++) {
        	//Iterator<E> successors = mapEdges.get(edges.get(i).id()).successors();
            Iterator<E> successors = edges.get(i).successors();
            boolean successor = false;

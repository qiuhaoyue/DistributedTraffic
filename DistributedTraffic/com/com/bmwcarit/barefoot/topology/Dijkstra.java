 ...
 */
public class Dijkstra<E extends AbstractEdge<E>, P extends Point<E>> implements Router<E, P>, Serializable{
    /**
	 * 
	 */
    ...
    public Map<P, List<E>> route(HashMap<Long, Tuple2<Road,Road>> successorAndNeighbor, P source, Set<P> targets, Cost<E> cost, Cost<E> bound,
			Double max) {
    	return ssmt(successorAndNeighbor,source, targets, cost, bound, max);
	}
    @Override
    ...
    private Map<P, List<E>> ssmt(HashMap<Long, Tuple2<Road,Road>> successorAndNeighbor, P source, Set<P> targets, Cost<E> cost, Cost<E> bound, Double max) {
        Map<P, Tuple<P, List<E>>> map =
                msmt(successorAndNeighbor, new HashSet<P>(Arrays.asList(source)), targets, cost, bound, max);
    ...
    private Map<P, Tuple<P, List<E>>> msmt(HashMap<Long, Tuple2<Road,Road>> successorAndNeighbor, final Set<P> sources, final Set<P> targets,
        ...
        while (priorities.size() > 0) {
            ...
            //修改
            Iterator<E> successors = current.one().successors(successorAndNeighbor);
    ...
    private Map<P, Tuple<P, List<E>>> msmt(final Set<P> sources, final Set<P> targets,
        ...
        while (priorities.size() > 0) {
            ...
            logger.trace("succeed edge {} with {} cost", current.one().id(), current.three());
            //修改
            //Iterator<E> successors = this.edges.get(current.one().id()).successors();
            Iterator<E> successors = current.one().successors();
